...
function openInvite(client){const form=el("form","ap-stack");const email=input("Client email","email");email.value=client.contact_email||"";const submit=button("Create invitation","ap-btn ap-btn--primary");submit.type="submit";const result=el("div","ap-invite-result");form.append(email,submit,result);form.addEventListener("submit",async e=>{e.preventDefault();submit.disabled=true;try{const inv=await inviteMember({email:email.value,role:"client",clientId:client.id});result.replaceChildren();result.append(el("p","","Copy this invitation link:"),el("code","",inv.link));const copy=button("Copy link","ap-btn ap-btn--secondary");copy.type="button";copy.addEventListener("click",()=>navigator.clipboard.writeText(inv.link).then(()=>AppCore.ui.toast("Invitation link copied.","success")));result.append(copy);}catch(err){AppCore.ui.toast(AppCore.ui.describeError(err),"error");}finally{submit.disabled=false;}});AppCore.ui.openModal({title:`Invite to ${client.name}`,content:form,actions:[]});}
