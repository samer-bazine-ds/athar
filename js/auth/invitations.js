...
function invitationLink(token,role){const url=new URL("login.html",window.location.href);url.searchParams.set("invite",token);if(role==="client")url.searchParams.set("client_access","launch");return url.toString();}
...
async function createInvitation(payload){
  let {data,error}=await supabase.from("invitations").insert(payload).select().single();
  if(!error)return{data,resent:false};
  if(error.code!=="23505")throw error;
  const {data:pending,error:pendingError}=await supabase.from("invitations").select("*").eq("agency_id",payload.agency_id).eq("email",payload.email).eq("status","pending").maybeSingle();
  if(pendingError||!pending)throw error;
  const {error:revokeError}=await supabase.from("invitations").update({status:"revoked"}).eq("id",pending.id).eq("status","pending");
  if(revokeError)throw revokeError;
  ({data,error}=await supabase.from("invitations").insert(payload).select().single());
...
export async function inviteMember({email,role="team",clientId=null}){
  ...
  const {data,resent}=await createInvitation(payload);
  const link=invitationLink(data.token,role);
  ...
  return {...data,link,emailSent:true,resent};
}
export async function startClientAccess(token){const {data,error}=await supabase.functions.invoke("start-client-access",{body:{token}});if(error)throw error;if(!data?.action_link)throw Object.assign(new Error(data?.error||"Could not start client access."),{userMessage:true});location.replace(data.action_link);}
